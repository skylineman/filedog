...
class PhoneCodeAuthProvider extends AuthProvider {
  ...
  Future<bool> sendPhoneCode( { required String? phoneNumber } ) async {
    ...
    final CloudBaseResponse? res = await CloudBaseRequest(super.core).postWithoutAuth(
      'auth.sendPhoneCode', {'phoneNumber': phoneNumber }
    );
  ...
  Future<CloudBaseAuthState> signInWithPhoneCode(
    ...
    if (res.data != null && res.data['refresh_token'] != null) {
      ...
      if (res.data['access_token'] != null && res.data['access_token_expire'] != null) {
        ...
        await cache.setStore(
          cache.accessTokenExpireKey,
          res.data["access_token_expire"] + DateTime.now().millisecondsSinceEpoch
        );
      } else {
        await refreshAccessToken();
      }
      return CloudBaseAuthState( authType: CloudBaseAuthType.PHONECODE, refreshToken: refreshToken );
    }
    else {
      // 短信登录失败
      //debugPrint( 'SMS ERROR: ' + res.message! + ' code: ' + res.code.toString() + 'data: ' + res.data.toString() );
      throw CloudBaseException( code: CloudBaseExceptionCode.AUTH_FAILED, message: '短信登录失败' );
    }
  }
