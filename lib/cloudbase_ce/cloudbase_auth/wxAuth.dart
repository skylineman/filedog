...
class WxAuthProvider extends AuthProvider {
  ...
  Future<CloudBaseAuthState> signInByWx( String? wxAppId, String? wxUniLink) async {
    ...
    if (res.data != null && res.data['refresh_token'] != null) {
      ...
      throw CloudBaseException( code: CloudBaseExceptionCode.AUTH_FAILED, message: '微信登录失败');
    }
  }
  ...
  Future<String> _getWxCode(String wxAppId, String wxUniLink) async {
    ...
    final String code = await _channel.invokeMethod('wxauth.login').catchError((e) {
      throw CloudBaseException( code: CloudBaseExceptionCode.AUTH_FAILED, message: e.message );
    });
